...
test('Crear usuario no valido (nombre de 16 letras)', async ({ page }) => {
  ...
  await page.fill('input[name="first_name"]', 'José');
  await page.fill('input[name="last_name"]', 'González Ramírez');
  ...
  // Otros campos
  await page.fill('input[name="email"]', 'TC12@gmail.com');
 ...
 // Espera a que aparezca el mensaje de error en pantalla
  await expect(page.locator('text=Error al crear el usuario')).toBeVisible({ timeout: 5000 });
});