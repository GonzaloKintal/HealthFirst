...
test('Crear usuario no válido (Contraseña vacía)', async ({ page }) => {
  ...
  await page.fill('input[name="password"]', ''); // ❌ Contraseña vacía
  ...
  // Validar mensaje nativo del navegador para campo vacío
  const passwordMessage = await page.$eval('input[name="password"]', el => el.validationMessage);
  expect(passwordMessage).toMatch(/completa este campo/i); // o "fill out this field" según idioma
