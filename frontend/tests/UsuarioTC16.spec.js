...
test('Crear usuario no valido (DNI vacío))', async ({ page }) => {
  ...
  await page.fill('input[name="dni"]', '');
  ...
  // Validar que el campo DNI no es válido (HTML5)
  const isValid = await page.$eval('input[name="dni"]', el => el.checkValidity());
  expect(isValid).toBe(false);

  // Capturar mensaje del navegador
  const validationMessage = await page.$eval('input[name="dni"]', el => el.validationMessage);
  console.log('Mensaje del navegador:', validationMessage);

  // Validar mensaje (esto puede cambiar según el idioma del navegador)
