...
test('Crear usuario no valido (apellido vacío)', async ({ page }) => {
  ...
  // Otros campos
  await page.fill('input[name="email"]', 'TC10@gmail.com');
  ...
  const isValid = await page.$eval('input[name="last_name"]', el => el.checkValidity());
  expect(isValid).toBe(false);

  // También podés verificar el mensaje del navegador (opcional)
  const message = await page.$eval('input[name="last_name"]', el => el.validationMessage);
