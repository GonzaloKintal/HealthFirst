...
test('"Crear usuario válido (La Fecha de Nacimiento es de una persona de edad borde, 18 años))', async ({ page }) => {
  ...
  await page.selectOption('select.react-datepicker__year-select', '2007'); // Selecciona el año 2005 para que la persona tenga 18 años
  await page.locator('.react-datepicker__day--026:not(.react-datepicker__day--outside-month)').click();

  await page.waitForTimeout(2000); // Esperar a que se procese la fecha
  // Otros campos
  await page.fill('input[name="email"]', 'TC20@gmail.com');
